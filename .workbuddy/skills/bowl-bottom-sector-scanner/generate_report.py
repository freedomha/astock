#!/usr/bin/env python3
"""
生成 A股题材碗底形态分析 HTML 报告 (增强版 v2)
- 摘要卡片 (确认/筑底/下跌中继/观望)
- 碗底确认题材 120日 K线缩略图 (Chart.js)
- TOP25 综合得分排名表
- 碗底确认题材详细分析卡片
"""
...
def build_report(results, klines, output_path):
    ...
    # Build chart datasets for top confirmed sectors
    chart_blocks = []
    ...
    for i, r in enumerate(confirmed[:9]):
        ...
        <div class="detail-card">
          <div class="detail-head">
            ...
            <h3>{r['name']} <span class="muted">({r['type']})</span></h3>
            <span class="badge" style="background:#27ae60">{r['label']}</span>
            <span class="score-badge">得分 {r['score']}/100</span>
          </div>
          <div class="detail-grid">
            <div class="metric"><span class="ml">当前点位</span><span class="mv">{r['current']}</span></div>
...
<title>A股题材碗底形态分析报告 · {today}</title>
...
<div class="header">
  <h1>🥣 A股题材板块碗底形态分析报告</h1>
  <div class="subtitle">按"碗底形状"筛选处于底部区间的题材 · 申万一级行业 + 聚源产业概念板块</div>
  <div class="meta">
    <span>📅 数据日期: {today}</span>
    <span>📊 样本: {total} 个板块</span>
...
<div class="summary-cards">
  <div class="summary-card c-blue"><div class="number">{total}</div><div class="label">分析题材总数</div></div>
...
<div class="note">
  <b>📌 碗底判定逻辑 (增强版 v2):</b> 真碗底 = ① 处于 120/250 日区间底部 (位置≤25%) + ② 前期明显下跌后近期趋稳/走平 (减速比&lt;0.8) + ③ 低点抬升 (近10日低点&gt;前10日低点)。
  <b>减速比</b> = 近20日跌幅 ÷ 前40日(等效20日)跌幅，越小代表跌势减速越明显 (0≈已止跌)。
  本算法会剔除"仍在创新低"的伪碗底 (如近期破位下跌的题材)。
</div>

<div class="section-title">🏆 碗底确认题材 — K线缩略图 <span class="count">(展示前9名 120日走势)</span></div>
...
<table>
<thead><tr>
<th>#</th><th>板块</th><th>形态判定</th><th>得分</th><th>120日位</th><th>250日位</th><th>近20日</th><th>前40日</th><th>减速比</th><th>低点</th><th>120日回撤</th><th>距60MA</th>
</tr></thead>
<tbody>
{''.join(table_rows)}
</tbody></table>
...
<div class="section-title">📝 碗底确认题材详细分析</div>
{''.join(detail_cards)}

<div class="disclaimer">
  ⚠️ <b>风险提示:</b> 本报告仅基于历史价格形态的客观量化分析，不构成任何投资建议。碗底形态识别属于技术分析方法，存在误判可能；
  处于底部区间的题材可能继续下跌 (底部之下还有底)。投资有风险，决策需谨慎。请结合基本面、资金面、宏观环境综合判断。
...
chartData.forEach((c, idx) => {{
  ...
  card.innerHTML = `
    <h4><span style="color:${{colors[idx%colors.length]}}">●</span> ${{c.name}} <span class="muted">${{c.type}}</span></h4>
  ...
  new Chart(ctx, {{
    ...
    options: {{
      ...
      plugins: {{ legend:{{display:false}}, tooltip:{{ callbacks:{{ title:(i)=>c.dates[i[0].dataIndex], label:(i)=>'点位 '+i.parsed.y }}}}}},
...
def main():
    ...
    output = os.path.join(cwd, "sector_bowl_report.html")
