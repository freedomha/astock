...
> **低吸 vs 建仓边界（SOP 硬约束）**：本 skill 的**门口 = T2 / T3a 小额试仓**（每笔开目标仓位的 **25%**）。完整建仓（分批增加核心仓）只允许 **T3b / T4** 白名单。T2 仅为小额试仓、T3a 需「小额、等待回踩」（MA60 仍向下，必须等 MA60 走平/转上才可加码）。**禁止在 T0/T1 抄底**。四个校验任一失败 → 自动降级 HOLD。
> **默认目标仓注入**：未在 `records/portfolio_config.json` positions 中定义的候选（扫描新发现的标的），若低吸分≥50 且 RR 达标，自动注入默认 **25% 目标仓**（role=tactical_value, max=25%, risk_budget=2.0%，与四只战术仓同口径），试仓金额=目标仓的 **25%**，据此计算交易成本与订单手数——避免因无目标权重算出 0 手被成本门误拦。磁盘上的 portfolio_config.json 不会被修改。
...
- 所有金额/权重/成本参数一律读取 `records/portfolio_config.json`，**绝不硬编码**（唯一例外：未配置候选按上方「默认目标仓注入」规则使用内置默认值）
...
- **`verdict = "PASS"`** → 四道校验全过，`order` 给出建议 **小额试仓手数/金额**（=目标仓 25%）
...
2. **风险收益（成本调整后 + 止损前置）**：无结构失效位 → 拒绝；`rr_net = (第一观察区 − 计划买入价 − 每股成本) / (计划买入价 − 结构失效价)`。**小额试仓(T2/T3a, ≤25%目标仓)用更低下限 RR≥1.2**（极小仓位换取更宽 RR）；完整建仓仍须 RR≥2 → 拒绝。
...
| **T2** | **底部构建** | **pos250≤35% + 低点抬高 + 周线非向下** | **✅ 小额试仓(25%目标仓)** |
...
- **`PASS`（可小额试仓）**：四道校验全过，可按 `order.shares/amount` 开 **25% 目标仓**的试仓单。仍非建仓——周线升级 T3b 后再按 `etf-operation-plan` 分批建仓节奏加仓。
...
- **门口 = T2/T3a 小额试仓（25% 目标仓）**，不是完整建仓。完整建仓（分批增加核心仓）仅限 T3b/T4，且须通过操作引擎四道程序校验。
