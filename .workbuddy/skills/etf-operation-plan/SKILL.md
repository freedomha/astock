...
3. **状态机 + 防抖动（可选变体）** — 趋势状态有历史持久化与合法迁移规则，避免周线边界来回切换。⚠️ 「普通升级需连续 2 个完整周线确认」目前**只在 `backtest.py --use-confirmed` 中实现为暴露门控**；实盘 `migrate()` 不做门控，只输出 `confirmation` 字段供解读（详见「状态机与合法迁移」）。已 A/B 实测：防抖动降低换手/波动/成本，但收益下降、夏普持平，**未上调为默认**。
...
- **⚠️ 口径澄清（易误读）**：`confirmation` 是**报告字段，不是门控**。
  - `trend_analysis.py` 的 `migrate()` 对 `normal_advance` / `jump` / `degradation` **一律立即把新状态作为 `effective_state` 返回**，仅把 `confirmation` 标为 `pending`。**实盘路径不做确认门控 —— 未满 2 周也会生效。**
  - 真正执行「2 周确认」门控的只有 `backtest.py` 的 `build_daily_exposure()`，且**仅在 `--use-confirmed` 开启时**生效，并且**只拦截暴露上调**（`new_exp > old_exp`）；**下调与 T8 一律立即生效**。
  - **A/B 实测结论（2026-09-17，346 ETF × 500 日）**：防抖动的增益是**风险维度**的、而非收益维度的，且幅度不大 ——

    | 指标 | 默认（即时生效） | `--use-confirmed` | 差 |
    |---|---|---|---|
    | 年化收益 | 0.96% | 0.29% | **-0.67pp（变差）** |
    | 最大回撤 | -17.54% | -16.87% | +0.67pp（略好） |
    | 年化波动 | 18.08% | 16.70% | -1.38pp（更好） |
    | 夏普 | -0.04 | -0.02 | +0.02（基本持平） |
    | 平均换手次数 | 127.6 | 110.0 | -17.6（更少） |
    | 平均成本拖累 | +2.85% | +2.38% | -0.47pp（更省） |
    | 跑赢买入持有占比 | 28.6% | 32.2% | +3.6pp（更好） |

    两类口径**均大幅跑输买入持有**（年化 8.39%），符合「操作计划是风险控制型框架」的定位。防抖动确实减少了来回打脸（换手 -14%、成本拖累 -0.47pp、波动 -1.38pp），但代价是收益下降、夏普基本没改善。**故暂不上调为实盘默认**，维持「可选变体」定位。
  - **解读 `pending` 的正确方式**：视为「该状态尚未获 2 周确认」的风险提示，**而非**「状态未生效」。下发操作建议时须显式标注此歧义：若 pending 的新旧状态指向**相同动作**（如 T1 与 T6 都禁新增），可直接按新状态执行；若指向**不同动作**（尤其涉及降仓/退出），应按 pending 给出**分段执行**方案，而非一次性全额执行。
...
7. **操作计划** — 仓位动作（核心/战术/预留，受硬约束矩阵）、分批触发、不应采取的动作、最大可接受风险。**小节标题与触发措辞必须按 role 动态命名**：全部为核心仓时用「核心仓动作」，不得出现「战术仓」字样；存在战术仓时再用「战术仓位动作」。
...
- **周线用完整自然交易周**，当前周仅预览
- **状态机防抖动是可选变体**：2 周确认门控只在 `backtest.py --use-confirmed` 中实现（且只拦暴露上调，下调与 T8 立即生效）；实盘 `migrate()` 立即生效、仅报告 `confirmation` 字段。`pending` 应作为风险提示而非「未生效」，动作有分歧时给分段执行方案
