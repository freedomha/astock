...
def detect_2b_bottom(records):
    ...
    # Step 2: Check breakdown on bars 0, 1, 2
...
def quality_filter_2b(records, breakdown_bar, recovery_bar, prior_low_price, prior_low_bar):
    """
    v3 Quality pre-filter: filters ~74% of low-quality 2B signals.
    
    Returns (passed, reason, metrics_dict).
    """
    n = len(records)
    closes = [r["close"] for r in records]
    vols = [r["volume"] for r in records]

    # C1: Recovery strength
...
def score_2b(records, breakdown_bar, recovery_bar, prior_low_price, prior_low_bar,
    ...
    v3: precomputed metrics from quality_filter_2b() avoid recomputation.
        Shallow breaks (<1%) get LOW points, deep breaks (1-5%) get HIGH points.
    """
    n = len(records)
    closes = [r["close"] for r in records]
    highs = [r["high"] for r in records]
    ...
    # Dimension 1: Break depth (max 20) — v3 inverted
...
def analyze_2b(code, name, etype, kline_data):
    ...
    v3: Quality pre-filter before scoring. Filters ~74% of low-quality signals.
    """
    if not kline_data:
        return None
